    ...
    >
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-10 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm" style={{ color: "var(--color-muted-foreground)" }}>
              {locale === "zh"
                ? "上传商品照片，选择输出方向"
                : "Upload product photos and choose your output"}
            </p>
          </div>
          <Link
            href={`/${locale}/pricing`}
        ...
        <div
          ...
          onDrop={handleDrop}
          className={`relative rounded-[1.5rem] border-2 border-dashed p-12 text-center transition-all duration-200 ${
            dragOver ? "scale-[1.02] border-indigo-400 bg-indigo-50/30" : ""
          }`}
          style={{
            borderColor: dragOver ? undefined : "var(--color-card-border)",
              ...
              {files.map((file, i) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 flex h-6 w-6 items-center justify-center rounded-full bg-black/60 text-xs text-white opacity-0 transition-opacity group-hover:opacity-100"
                  >
                    ✕
                  </button>
                  <p
          ...
          <div className="mt-10">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                {
                  icon: "🖼️",
                  title: locale === "zh" ? "电商主图" : "Hero Image",
                  desc: locale === "zh" ? "适配各平台尺寸规范" : "Platform-optimized",
                },
                {
                  icon: "📋",
                  title: locale === "zh" ? "详情页" : "Detail Page",
                  desc: locale === "zh" ? "多模块批量生成" : "Multi-module batch",
                },
                {
                  icon: "🎬",
                  title: locale === "zh" ? "商品视频" : "Product Video",
                  desc: locale === "zh" ? "Seedance 2.0" : "Seedance 2.0",
                },
              ].map((opt) => (
                <button
                  key={opt.title}
